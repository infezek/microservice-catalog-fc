...
describe("ListCategoriesUseCase Integration Tests", () => {
  let useCase: ListCategories.UseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new ListCategories.UseCase(repository);
  });
  ...
  it("should return output using empty input with categories ordered by created_at", async () => {
    ...
    const entities = faker
      ...
      .withCreatedAt(
        (index) => new Date(new Date().getTime() + index)
      )
      .build();
  ...
  it("should returns output using pagination, sort and filter", async () => {
    ...
    expect(output).toMatchObject({
      items: [entities[1], entities[2]].map((i) =>
        i.toJSON()
      ),
      total: 3,
      current_page: 1,
    ...
    expect(output).toMatchObject({
      items: [entities[0], entities[2]].map((i) =>
        i.toJSON()
      ),
      total: 3,
      current_page: 1,
