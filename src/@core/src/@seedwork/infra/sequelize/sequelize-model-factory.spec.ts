...
describe("SequelizeModelFactory Unit Tests", () => {
  ...
  test("create method", async () => {
    ...
    model = await StubModel.factory().create({
      id: "4cfd293c-8d59-5d16-8f50-6fca993ca7f3",
      name: "test",
    });
    expect(model.id).toBe(
      "4cfd293c-8d59-5d16-8f50-6fca993ca7f3"
    );
  ...
  test("make method", async () => {
    ...
    model = StubModel.factory().make({
      id: "8734eb15-f280-5149-9d49-1abd89fdd63d",
      name: "test",
    });
    expect(model.id).toBe(
      "8734eb15-f280-5149-9d49-1abd89fdd63d"
    );
    expect(model.name).toBe("test");
    expect(StubModel.mockFactory).toHaveBeenCalledTimes(1);
  });
  ...
  test("bulkCreate method using count > 1", async () => {
    let models = await StubModel.factory()
      .count(2)
      .bulkCreate();
    ...
    let modelFound1 = await StubModel.findByPk(
      models[0].id
    );
    expect(models[0].id).toBe(modelFound1.id);
    expect(models[0].name).toBe(modelFound1.name);

    let modelFound2 = await StubModel.findByPk(
      models[1].id
    );
